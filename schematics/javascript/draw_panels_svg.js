(function ($) {
    $.fn.draw_panel = function (panel_objs, options) {

        // example panel objs
        // var panel_objs = [[0,0,32,48,'standard'],[0,0,32,48,'standard']];
        ...
        return this.each(function () {
            ...
            function new_canvas(w,h) {
                ...
                uid = 'panel_'+Math.floor((Math.random()*100)*(Math.random()*100)*(Math.random()*100));
                sel_obj.append('<div id="'+uid+'" class="drawn_panel"></div>');
                paper = Raphael(document.getElementById(uid), (w+(padding*2)), (h+(padding*2)));
            ...
            function hole_line(piece_x,piece_y,length,inset,direction,prevent_mockup) {
                ...
                switch(direction) {
                    case 'l2r':
                        ...
                        // right inset
                        draw_hole(piece_x+length-inset, piece_y+pvc_center, radius);
                    break;
                    case 'r2l':
                        // mock it up
                        if (!prevent_mockup) { draw_pvc(piece_x,piece_y,length,holes.pvc); }
                        ...
                        // right inset
                        draw_hole(piece_x+inset, piece_y+pvc_center, radius);
                    break;
                    case 't2b':
                        // mock it up
                        ...
                        // top inset
                        draw_hole(piece_x+pvc_center, piece_y+length-inset, radius);
                    break;
                    case 'b2t':
                        // mock it up
                        if (!prevent_mockup) { draw_pvc(piece_x,piece_y,holes.pvc,length); }
                        ...
                        // top inset
                        draw_hole(piece_x+pvc_center, piece_y+inset, radius);
                    break;
                    default:
            ...
            function draw_panel(x,y,w,h,radius,panel_type,door_w,door_h,door_in_left) {
                // draw the surface
                ...
                if (panel_type === 'dropdown' ) {
                    ...
                    if (door_in_left === 0) {
                        ...
                        if (door_w === w) {
                            ...
                            // right side (sq)
                            hole_line(x+w-holes.pvc, y+door_h+holes.lc_thick, h-door_h-holes.lc_thick, holes.pn_l_in, 'b2t');
                            // door right side (lc)
            ...
            // drawing [panel]
            for (var i = 0; i < panel_objs.length; i++) {
                var this_panel = panel_objs[i];
                var x = this_panel[0]*px_scale,
                    y = this_panel[1]*px_scale,
                    w = this_panel[2]*px_scale,
                    h = this_panel[3]*px_scale,
                    panel_type = this_panel[4],
                    dd_w = this_panel[5]*px_scale,
                    dd_h = this_panel[6]*px_scale,
                    dd_in = this_panel[7]*px_scale;
                new_canvas(w,h);
                if (panel_type === 'dropdown') {
                ...
                function encode_as_img_and_link(){
                    ...
                    $($('#'+uid)).each(function() {
                        ...
                        // link_container.append($("<span class='link'><a href-lang='image/svg+xml' download='panel_"+count+".svg' href='data:image/svg+xml;base64,\n"+b64+"' title='panel_"+count+".svg'>panel_"+count+".svg</a></span>"));
                        svg_elem.hide();
                    });
                }
                // $(this).prepend('<div class="panel_link_container"></div>');
                // var link_container = $(this).children('.panel_link_container');
